...
class SearchView(BaseView):
    ...
    def callback_add_new_search(self):
        ...
        if new_search:
            # Optional: ruleaza un prim scrape imediat
            # new_search.job_search() # Acest lucru poate bloca UI-ul daca e sincron
            # Considera rularea intr-un thread daca job_search e de durata
            
            # Astra treubie cu thread ca sa nu blocheze toata aplicatia
            new_search.job_search()
            self.update_search_listbox_display()
    ...
    def on_select_search(self, event):
        ...
        for job_obj in jobs_dict.values(): # Iteram prin valorile dictionarului (obiectele Job)
